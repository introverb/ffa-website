    ...
    <div className={`relative ${soldOut || artwork.status === 'reserved' ? 'opacity-60' : ''}`}>
      ...
      <div className="mt-4 flex flex-col items-start gap-3">
        ...
        {label === 'Reserved' ? (
          ...
          // Opens the purchase modal (image + checkout info); the modal
          // posts the same plain HTML form to the server-side Checkout
          // Session creator that the card itself used to.
          <BuyModal artwork={artwork} />
        ) : null}
      </div>
    </div>
  );
